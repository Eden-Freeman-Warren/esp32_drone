#!/usr/bin/env python3
"""
Autonomous ESP32-S3 ESP-Drone hover with HOME-yaw coordinates.
...
Keep together:
    espnow_test_ready_v6_home_yaw.py
    espnow_driver.py
    NatNetClient.py
"""
...
latest_world_quaternion = None
latest_sample_time = 0.0

home_position = None
home_yaw_radians = None
...
def normalize_quaternion(quaternion):
    ...
    if magnitude < 1.0e-9:
        raise ValueError("OptiTrack quaternion has zero length.")
...
def quaternion_to_yaw(quaternion):
    ...
    NatNet supplies quaternion order:
        x, y, z, w

    Positive yaw is counterclockwise when viewed from above. With your
    rigid-body axes, +X is forward, +Y is left, and +Z is up.
    """
...
def receive_rigid_body(
    ...
    *extra,
    ...
    try:
        ...
        quaternion_sample = normalize_quaternion(quaternion)
    except (TypeError, ValueError, IndexError):
        return

    with data_lock:
        latest_world_position = position_sample
        latest_world_quaternion = quaternion_sample
...
def read_pose():
    with data_lock:
        ...
        quaternion = latest_world_quaternion
...
def read_tracking():
    position, _, age = read_pose()
    return position, age


def local_position(world_position):
    """
    Convert Motive world coordinates to the takeoff frame.

    Output frame:
        +X = drone forward at HOME
        +Y = drone left at HOME
        +Z = up

    This is a rotation by negative HOME yaw.
    """
    with data_lock:
        home = home_position
        home_yaw = home_yaw_radians

    if home is None or home_yaw is None:
        raise RuntimeError("HOME pose has not been captured.")

    dx_world = world_position[0] - home[0]
    dy_world = world_position[1] - home[1]
    dz_world = world_position[2] - home[2]

    cosine = math.cos(home_yaw)
    sine = math.sin(home_yaw)

    x_forward = cosine * dx_world + sine * dy_world
    y_left = -sine * dx_world + cosine * dy_world

    return x_forward, y_left, dz_world
...
def capture_home():
    ...
    global home_yaw_radians

    print("Keep the drone still and level while HOME is captured.")
    print("Rigid-body +X must point forward and +Y must point left.")
    ...
    while time.monotonic() < deadline:
        position, quaternion, age = read_pose()

        if (
            position is None
            or quaternion is None
            or age > TRACKING_TIMEOUT
        ):
            raise RuntimeError(
        ...
        for axis in range(3):
            position_sums[axis] += position[axis]

        yaw = quaternion_to_yaw(quaternion)
        yaw_sine_sum += math.sin(yaw)
        yaw_cosine_sum += math.cos(yaw)
    ...
    captured_position = (
        position_sums[0] / count,
        position_sums[1] / count,
        position_sums[2] / count,
    )
    captured_yaw = math.atan2(
        yaw_sine_sum,
        yaw_cosine_sum,
    )

    with data_lock:
        home_position = captured_position
        home_yaw_radians = captured_yaw

    print(
        "HOME captured: "
        f"({captured_position[0]:+.3f}, "
        f"{captured_position[1]:+.3f}, "
        f"{captured_position[2]:+.3f}) m"
    )
    print(
        f"HOME yaw: {math.degrees(captured_yaw):+.1f} degrees"
    )
    print(
        "Local axes are now: +X forward, +Y left, +Z up."
    )
...
def send_esp_drone_position_setpoint(
    crazyflie,
    x_m,
    y_m,
    z_m,
    ...
    """
    Use ESP-Drone's legacy RPYT POSSET mode.

    Firmware mapping in crtp_commander_rpyt.c:
        position.x = -pitch
        position.y = roll
        position.z = thrust / 1000.0
        yaw        = yaw

    A nonzero thrust field selects POSSET mode. It is a Z coordinate in
    millimeters here, not a motor-thrust command.
    """
    z_m = max(0.001, float(z_m))
    z_millimeters = max(1, min(65535, round(z_m * 1000.0)))

    crazyflie.commander.send_setpoint(
        float(y_m),             # roll -> position Y
        float(-x_m),            # pitch -> position X
        float(yaw_degrees),     # absolute yaw in POSSET mode
        int(z_millimeters),     # position Z in millimeters
    )


def unlock_legacy_commander(crazyflie):
    """
    ESP-Drone's legacy commander starts thrust-locked. Zero-thrust packets
    clear that lock before POSSET packets are sent.
    """
    print("Unlocking legacy commander...")
...
def run_position_segment(
    ...
    for step in range(steps + 1):
        ...
        target_z = max(0.001, target_z)

        send_esp_drone_position_setpoint(
            crazyflie=crazyflie,
            x_m=0.0,
            y_m=0.0,
            z_m=target_z,
...
def main():
    ...
    try:
        ...
        with SyncCrazyflie(
            ...
            # ESP-Drone has a firmware-specific POSSET mode on the proven
            # legacy commander port (the same port as the working thrust test).
            set_parameter(cf, "commander.enHighLevel", "0")
            set_parameter(cf, "flightmode.posSet", "1")

            # Conservative horizontal controller settings.
            # Conservative horizontal position control.
            set_parameter(cf, "posCtlPid.rpLimit", "3.0")
            set_parameter(cf, "posCtlPid.xyVelMax", "0.15")
            set_parameter(cf, "posCtlPid.xKi", "0.0")
            set_parameter(cf, "posCtlPid.yKi", "0.0")

            # Reduce the strength of horizontal velocity corrections.
            set_parameter(cf, "velCtlPid.vxKp", "10.0")
            set_parameter(cf, "velCtlPid.vyKp", "10.0")
            set_parameter(cf, "velCtlPid.vxKi", "0.0")
            set_parameter(cf, "velCtlPid.vyKi", "0.0")

            # Starting thrust for the heavier battery.
            set_parameter(cf, "posCtlPid.thrustBase", "53000")
            ...
            # At 1 cm, stop the POSSET stream and explicitly command zero.
            for _ in range(10):
                cf.commander.send_setpoint(0.0, 0.0, 0.0, 0)
                time.sleep(0.05)

            cf.commander.send_stop_setpoint()
            set_parameter(cf, "flightmode.posSet", "0")
